...
const Login: React.FC = () => {
  ...
  const { login, loginWithGoogle, loginWithGithub, error, loading, clearError } = useAuth();
  const navigate = useNavigate();
  const location = useLocation();
  
  // Check if there's a GitHub code in the URL (for GitHub OAuth callback)
  useEffect(() => {
  ...
  const handleGitHubCallback = async (code: string) => {
    try {
      ...
      if (success) {
        ...
        setSnackbarMessage('GitHub login failed. Please try again.');
        setSnackbarOpen(true);
      }
    } catch (err) {
      setSnackbarSeverity('error');
  ...
  const handleGoogleLogin = async () => {
    // In a real app, you would integrate with Google's OAuth API
